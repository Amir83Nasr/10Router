...
const proxyClientMaxBodySize = process.env.TENROUTER_PROXY_CLIENT_MAX_BODY_SIZE || "128mb";
...
const nextConfig = {
  ...
  // Incremental TS migration: don't fail the build on the growing set of
  // JS files still being converted (route handlers return non-Response objects,
  // etc.). `tsc --noEmit` stays the source of truth; tighten once migration landss.
  typescript: {
  ...
  webpack: (config, { isServer }) => {
    ...
    config.watchOptions = {
      ...
      ignored:
        /[\\/](node_modules|\.git|logs|\.next|\.next-cli-build|gitbook|open-sse\.old|tests|docs)[\\/]/,
    };
    return config;
  },
  async rewrites() {
