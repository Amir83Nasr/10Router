...
function buildAgentRunFrame(messages, model) {
  ...
  const userMessage = concatBuffers(agentString(1, userText), agentString(2, crypto.randomUUID()));
...
function readCursorFrame(buffer, offset, frameNum, tag) {
  if (offset + 5 > buffer.length) {
    debugLog(
      `[CURSOR BUFFER${tag}] Reached end, offset=${offset}, remaining=${buffer.length - offset}`,
    );
    return { status: "done" };
  }
  ...
  debugLog(
    `[CURSOR BUFFER${tag}] Frame ${frameNum + 1}: flags=0x${flags.toString(16).padStart(2, "0")}, length=${length}`,
  );

  if (offset + 5 + length > buffer.length) {
    debugLog(
      `[CURSOR BUFFER${tag}] Incomplete frame, offset=${offset}, length=${length}, buffer.length=${buffer.length}`,
    );
    return { status: "done" };
  }
  ...
  async executeAgent({ model, body, stream, credentials, signal }) {
    ...
    if (status !== 200) {
      ...
      return {
        response: new Response(
          ...
          {
            status: status || HTTP_STATUS.SERVER_ERROR,
            headers: { "Content-Type": "application/json" },
          },
        ),
        url,
    ...
    if (stream === false) {
      ...
      if (agentError) {
        return {
          response: new Response(
            JSON.stringify({ error: { message: agentError, type: "api_error" } }),
            {
      ...
      start(controller) {
        consume((event) => {
          if (event.type === "text") {
            ...
            controller.enqueue(
              encoder.encode(sseChunk({ error: { message: event.value, type: "api_error" } })),
            );
            ...
            controller.enqueue(
              encoder.encode(
                chatChunkSse({ id: responseId, created, model, delta: {}, finishReason: "stop" }),
              ),
            );
            controller.enqueue(encoder.encode(SSE_DONE));
            controller.close();
