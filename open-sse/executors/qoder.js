...
function isBillingBlock(inner) {
  ...
  try {
    ...
    /* not JSON — fall through to legacy shape match */
  }
...
async function peekFirstQoderFrame(reader, decoder) {
  ...
  while (true) {
    ...
    if (statusVal !== 200) {
      return {
        isError: true,
        isBilling: isBillingBlock(inner),
        statusVal,
...
async function wrapQoderSSE(response, model, log = null) {
  ...
  if (peek.isError) {
    ...
    const status = peek.isBilling
      ...
      : Number.isInteger(peek.statusVal) &&
          peek.statusVal >= HTTP_STATUS.BAD_REQUEST &&
          peek.statusVal <= 599
        ? peek.statusVal
  ...
  const processLine = (line, controller) => {
    ...
    if (statusVal !== 200) {
      ...
      try {
        console.error(
          `[QODER] error envelope status=${statusVal} statusType=${typeof envelope.statusCodeValue} bodyType=${typeof envelope.body} body=${truncate(inner, 300)}`,
        );
      } catch {
