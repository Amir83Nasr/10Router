...
export async function handleChatCore({
  ...
  // Stable per-session id so all lines of one CLI conversation share a tag
  const sessionSeed = (() => {
    try {
  ...
  const clientRequestedStreaming =
    body.stream === true || sourceFormat === FORMATS.ANTIGRAVITY || sourceFormat === FORMATS.GEMINI;
  ...
  if (log?.line) {
    ...
    parts.push(`ACC:${acc}`);
    log.line(reqTag, "REQ", parts.join(" · "));
  }
  ...
  // Token-saver flags accumulator for the single "SAVERS" log line below.
  const xf = [];
  ...
  if (xf.length && log?.line) log.line(reqTag, "SAVERS", xf.join(" · "));
  ...
  if (
    ...
    try {
      ...
      if (newCredentials?.accessToken || newCredentials?.copilotToken) {
        if (log?.line) log.line(reqTag, "REFRESH", `token refreshed · ${provider}/${model}`);
