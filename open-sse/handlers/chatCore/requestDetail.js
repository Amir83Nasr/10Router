...
export function extractUsageFromResponse(responseBody) {
  ...
  // Gemini format. Antigravity wraps the payload in { response: {...} }.
...
// Plain tokens only — no ANSI, no emoji (ANSI is stripped by the log buffer,
// so colors emitted here would never survive to the terminal).
export function formatDoneLine({ usage, latency }) {
  ...
  if (cacheRead || cacheCreate) {
    const parts = [];
    if (cacheRead) parts.push(`read:${cacheRead}`);
    if (cacheCreate) parts.push(`create:${cacheCreate}`);
...
export function saveUsageStats({
  ...
  if (!silent) {
    ...
    console.log(
      `[${time}] [${label}] ${provider.toUpperCase()} | in=${inTokens} | out=${outTokens}${accountSuffix}`,
    );
  }
