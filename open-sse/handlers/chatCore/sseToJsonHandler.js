...
export async function handleForcedSSEToJson({
  ...
  if (isCodexResponsesApi) {
    try {
      ...
      if (sourceFormat === FORMATS.ANTIGRAVITY || sourceFormat === FORMATS.GEMINI) {
        finalResp = {
          response: {
            candidates: [
              {
      ...
      log?.error?.("CHAT", `Responses API SSE-to-JSON failed: ${err?.message || err}`);
  ...
  try {
    ...
    log?.error?.("CHAT", `Chat Completions SSE-to-JSON failed: ${err?.message || err}`);
    return createErrorResult(
      HTTP_STATUS.BAD_GATEWAY,
      "Failed to convert streaming response to JSON",
    );
