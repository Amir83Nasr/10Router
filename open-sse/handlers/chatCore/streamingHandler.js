...
export async function handleStreamingResponse({
  ...
  if (onRequestSuccess) {
    Promise.resolve()
      ...
      .catch((err) => {
        log?.warn?.("CHAT", `onRequestSuccess failed: ${err?.message || err}`);
      });
  }
  ...
  if (
    ...
    if (log?.errorLine)
      log.errorLine(
        reqTag,
        "BLOCKED",
        `${status} · ${provider}/${model} · non-SSE (${upstreamContentType})\n    ${shortMsg}`,
      );
    else console.error(`[stream] ${provider}/${model} blocked pipe: ${shortMsg} [${status}]`);
  ...
  saveRequestDetail(
    ...
    log?.warn?.("DB", `RequestDetail save failed: ${err.message}`);
  });
...
export function buildOnStreamComplete({
  ...
  const onStreamComplete = (contentObj, usage, ttftAt) => {
    ...
    saveRequestDetail(
      ...
      log?.warn?.("DB", `RequestDetail update failed: ${err.message}`);
    });
