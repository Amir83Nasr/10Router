...
export async function handleComboChat({
  ...
  for (let i = 0; i < rotatedModels.length; i++) {
    ...
    try {
      ...
      // Local routing error for a combo member (unknown name or nested combo
      // with no models, e.g. "Support"/"Last" = []): skip the member, don't
      // abort the whole combo with a misleading 400. Genuine upstream 400s
      // stay non-fallbackable per checkFallbackError.
      if (
        !shouldFallback &&
        result.status === 400 &&
        /invalid model format/i.test(errorText || "")
      ) {
        lastError = errorText || String(result.status);
        if (!lastStatus) lastStatus = result.status;
        log.warn("COMBO", `Model ${modelStr} invalid format, trying next`, {
      ...
      // For transient errors (500/502/503/504), wait for cooldown before falling through
