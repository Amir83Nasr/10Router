...
async function fetchCursorCatalog(credentials, signal, postFn = http2PostProto) {
  ...
  const response = await postFn(url, headers, new Uint8Array(), signal, FETCH_TIMEOUT_MS);
 ...
 */
export async function resolveCursorModels(credentials, options = {}, deps = {}) {
  ...
  try {
    // ponytail: test seam; production path always uses http2 (h2-only endpoint).
    const postFn = deps.postProto || http2PostProto;
    const models = await fetchCursorCatalog(credentials, options.signal, postFn);
