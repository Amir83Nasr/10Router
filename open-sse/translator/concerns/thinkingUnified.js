...
const FORMAT_TO_NATIVE = {
  ...
  vertex: "gemini-budget", // legacy key (provider removed) — gemini-family bodies
  antigravity: "gemini-budget",
  kiro: "kiro", // legacy key (provider removed)
...
// Gemini nests thinkingConfig under generationConfig. Antigravity wraps
...
function applyFormat(fmt, body, cfg, caps, supportedLevels, display) {
  ...
  switch (fmt) {
    ...
    case "kimi": {
      // legacy format key (provider removed) — kept for model-name compat
      if (none && canDisable) {
        body.thinking = { type: "disabled" };
        break;
      }
    ...
    case "kiro": // legacy (provider removed) — was system-tag injection, now no-op
      break;
    case "commandcode": {
