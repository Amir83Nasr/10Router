...
export function filterToOpenAIFormat(body, opts = {}) {
  ...
  body.messages = body.messages.map((msg) => {
    ...
    if (Array.isArray(msg.content)) {
      ...
      // Text-only arrays collapse to a plain string (many OpenAI-compatible
      // upstreams reject array content on plain chat turns).
      // ponytail: skip collapse when blocks carry cache_control (DashScope
      // explicit caching needs array form); collapse again if more fields arise.
      const hasCache = keepCache && filteredContent.some((b) => b.cache_control);
      if (!hasCache && filteredContent.every((b) => b.type === OPENAI_BLOCK.TEXT)) {
