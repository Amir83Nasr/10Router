...
import {
  ...
  DEFAULT_THINKING_CLOUD_CODE_SIGNATURE,
...
// OpenAI -> Cloud Code (Cloud Code Assist envelope)
export function openaiToCloudCodeRequest(model, body, stream, credentials = null) {
...
// Wrap Gemini format in Cloud Code wrapper (antigravity)
function wrapInCloudCodeEnvelope(model, cloudCode, credentials = null) {
  ...
  const envelope = {
    ...
    request: {
      ...
      contents: cloudCode.contents,
      systemInstruction: cloudCode.systemInstruction,
      generationConfig: cloudCode.generationConfig,
      tools: cloudCode.tools,
    },
  };

  if (cloudCode.tools?.length > 0) {
...
export function openaiToAntigravityRequest(model, body, stream, credentials = null) {
  ...
  const geminiCLI = openaiToCloudCodeRequest(model, body, stream);
