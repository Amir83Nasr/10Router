...
export default function BulkImportGrokCliModal({ isOpen, onClose, onSuccess }: any) {
  const [jsonText, setJsonText] = useState<string>("");
  const [submitting, setSubmitting] = useState<boolean>(false);
  const [parseError, setParseError] = useState<string>("");
  const [result, setResult] = useState<any>(null);
  const [isDragging, setIsDragging] = useState<boolean>(false);
  const [fileCountInfo, setFileCountInfo] = useState<any>(null);
  ...
  const processFiles = async (files: any) => {
    ...
    setParseError("");
    const jsonFiles: any[] = Array.from(files).filter(
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{translate("Bulk Add Grok CLI Accounts")}</DialogTitle>
        </DialogHeader>
