...
import { ArrowLeftRight, Server, Unplug } from "lucide-react";
...
export default function ProviderDetailPage() {
  ...
  const providerId: any = (params as any).id;
  ...
  const [showXiaomiMimoModal, setShowXiaomiMimoModal] = useState<boolean>(false);
  ...
  const [showBulkImportGrokCli, setShowBulkImportGrokCli] = useState<boolean>(false);
  ...
  const [liveModelsError, setLiveModelsError] = useState<any>(null);
  ...
  const oneByOne = useOneByOneTest(connections);
  const oneByOneRunning = oneByOne.running;
  const oneByOneStopping = oneByOne.stopping;
  const oneByOneCurrentConnectionId = oneByOne.currentConnectionId;
  const oneByOneResults = oneByOne.results;
  const oneByOneSummary = oneByOne.summary;
  const handleRunOneByOneTest = oneByOne.run;
  const handleStopOneByOneTest = oneByOne.stop;
  ...
  const apiKeyConnectionLabel =
    providerId === "xai" ? "xAI API Key" : providerId === "qoder" ? "PAT" : "API Key";
        ...
        <div key={conn.id} className="flex min-w-0 items-stretch">
          <div className="flex shrink-0 items-center pl-1 sm:pl-2">
            <Checkbox
              ...
              onCheckedChange={() => toggleSelectConnection(conn.id)}
            />
          </div>
  ...
  const bulkActionModal = (
    ...
    >
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Apply Proxy ({connections.length} connections)</DialogTitle>
        </DialogHeader>
        <div className="flex flex-col gap-2 p-4">
          <button
            onClick={handleApplyOneToOne}
            disabled={bulkUpdatingProxy || activePools.length === 0}
            className="flex items-center gap-3 rounded-lg border border-border px-3 py-2.5 text-left transition-colors hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50"
          >
            <ArrowLeftRight className="size-4 shrink-0 text-muted-foreground" />
            <span className="flex min-w-0 flex-col">
              <span className="text-sm font-medium text-foreground">One-to-one (rotate)</span>
              <span className="text-xs text-muted-foreground">
                Distribute connections across {activePools.length} active pool(s)
              </span>
            </span>
          </button>
          <button
          ...
          >
            ...
            <span className="flex min-w-0 flex-col">
              <span className="text-sm font-medium text-foreground">None (unbind all)</span>
              <span className="text-xs text-muted-foreground">
                Remove proxy from all connections
              </span>
            </span>
          </button>
          {proxyPools.map((pool) => (
    ...
    return (
      <div className="flex flex-wrap gap-3">
          ...
          <ModelRow
            ...
            onTest={
              connections.length > 0 || isFreeNoAuth
                ? () => handleTestModel(model.id, getModelKind(model, "llm"))
                : undefined
            }
        ...
        {providerId === "qoder" && connections.some((conn) => conn.isActive !== false) && (
          ...
          >
            <Icon
              name={importingQoderModels ? "progress_activity" : "download"}
              className="text-sm"
                    ...
                    <Tooltip key={m.id}>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-black/10 dark:border-white/10 text-xs text-muted-foreground hover:text-primary hover:border-primary/40 hover:bg-primary/5 transition-colors"
                        >
                ...
                <Tooltip key={m.id}>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-dashed border-black/10 dark:border-white/10 text-xs text-muted-foreground hover:text-primary hover:border-primary/40 hover:bg-primary/5 transition-colors"
                    >
                      <Icon name="add" className="text-[13px]" />
                      {m.id}
                    </button>
                  </TooltipTrigger>
                  <TooltipContent>Restore model</TooltipContent>
                </Tooltip>
              ))}
            </div>
          </div>
  ...
  // Determine icon path: guess a brand logo from the custom node baseUrl
  // first (e.g. api.deepseek.com → deepseek.png), then the API family logo.
  const getHeaderIconPath = () => {
    if (isCompatible) {
      const guessed = getProviderIconSrc(guessProviderIconIdFromUrl(providerNode?.baseUrl));
      if (guessed) return guessed;
      return isAnthropicCompatible ? "/providers/anthropic.png" : "/providers/openai.png";
    }
              ...
              {connections.length === 0 ? (
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex gap-2">
                    {hasDualAuthModes ? (
                      <>
                        <Button size="sm" variant="secondary" onClick={triggerOAuthConnection}>
                          <Icon name="lock" />
                          {oauthConnectionLabel}
                        </Button>
                        <Button size="sm" onClick={triggerApiKeyConnection}>
                          <Icon name="key" />
                      ...
                      <>
                        ...
                        <Button size="sm" onClick={triggerAddConnection}>
                          <Icon name="add" />
                          {isCompatible
                            ? "Add API Key"
                            : providerId === "iflow"
                ...
                <>
                  {oneByOneSummary && (
                    <div className="mb-4 rounded-lg border border-black/10 bg-black/[0.02] px-3 py-2 text-xs text-muted-foreground dark:border-white/10 dark:bg-white/[0.03]">
                  ...
                  {connections.length > 0 && (
                    <div className="mb-3 flex items-center gap-2 border-b border-black/[0.03] pb-2 dark:border-white/[0.03]">
                      <label className="flex cursor-pointer items-center gap-1.5 text-xs text-muted-foreground hover:text-primary">
                        <Checkbox
                          checked={allSelected}
                          onCheckedChange={toggleSelectAllConnections}
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-2 sm:flex">
                      {providerId === "iflow" && (
                        <Tooltip>
                          ...
                          <TooltipContent>Add connection using browser cookie</TooltipContent>
                        </Tooltip>
                      )}
                      {providerId === "codex" && (
                        <Tooltip>
                          ...
                          <TooltipContent>
                            {translate("Bulk import codex accounts from JSON")}
                          </TooltipContent>
                        </Tooltip>
                      )}
                      {providerId === "grok-cli" && (
                        <Tooltip>
                          ...
                          <TooltipContent>
                            {translate("Bulk import Grok CLI accounts from JSON")}
                          </TooltipContent>
                        </Tooltip>
                      )}
                      {hasDualAuthModes ? (
