      ...
      <CardContent className="flex flex-col gap-4 px-0 py-0">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-foreground">{name || provider}</h3>
              {plan && (
                <Badge variant={planVariants[plan?.toLowerCase()] || "default"}>{plan}</Badge>
              )}
            </div>
          </div>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <Icon
                  name="refresh"
                  className={`text-[20px] text-muted-foreground ${refreshing || loading ? "animate-spin" : ""}`}
                />
              </button>
            </TooltipTrigger>
