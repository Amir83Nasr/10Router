...
// ─── Pure helpers ─────────────────────────────────────────────────────────────
export function getConnectionLabel(connection: any) {
  return (
    connection.name?.trim() || connection.email?.trim() || connection.displayName?.trim() || null
  );
}

export function getConnectionQuotaRemaining(connection: any, quotaData: any) {
...
function groupByProviderStable(connections: any) {
  const seen = new Map();
...
export function buildLoadingState(connections: any) {
...
export function filterQuotaStateByConnections(state: any, connections: any) {
...
export function getConnectionsPageRange(pagination: any) {
  if (!pagination.total) {
...
export function getConnectionsEmptyMessage(totals: any, providerFilter: any, accountFilter: any) {
...
export function sortRequestFromExpiringFirst(expiringFirst: any) {
  return expiringFirst ? "expiring" : "priority";
}

export function getPageSizeLabel(pageSize: any, isCustomPageSize: any) {
  return isCustomPageSize ? `Custom: ${pageSize} / page` : `${pageSize} / page`;
}

export function getConnectionsPaginationSummary(pagination: any) {
  const { start, end } = getConnectionsPageRange(pagination);
  return `Showing ${start}-${end} of ${pagination.total}`;
}

export function getSafePagination(pagination: any, fallbackPageSize: any) {
  return (
    pagination || {
      page: 1,
...
export function getSafeTotals(totals: any, fallbackTotal = 0) {
  return (
    totals || {
...
export function shouldResetPage(previousValue: any, nextValue: any) {
  return previousValue !== nextValue;
}

export function getPaginationPageValue(dataPagination: any, fallbackPage: any) {
  return dataPagination?.page || fallbackPage;
}

export function getProviderOptions(dataProviderOptions: any) {
  return dataProviderOptions || [];
}

export async function reconcileConnectionsPage(fetchConnections: any, targetPage: any) {
...
export function setQuotaCache(connectionId: any, quotaEntry: any) {
  if (typeof window === "undefined") return;
  try {
...
export function formatResetTime(date: any) {
  ...
  try {
    const resetDate: any = typeof date === "string" ? new Date(date) : date;
    const now = new Date();
    const diffMs: any = (resetDate as any) - (now as any);
 ...
 * @returns {number} Remaining percentage (0-100)
 */
export function getRemainingPercentage(quota: any) {
...
export function getQuotaVisibilityKey(quota: any) {
...
function getProviderHiddenQuotaSet(provider: any, quotaVisibility: any, quotas = []) {
...
export function filterQuotasByVisibility(provider: any, quotas = [], quotaVisibility = {}) {
...
export function getHiddenQuotaRows(provider: any, quotas = [], quotaVisibility = {}) {
  if (!Array.isArray(quotas) || quotas.length === 0) return [];
  const hidden = getProviderHiddenQuotaSet(provider, quotaVisibility, quotas);
 ...
 * @param {string} provider - Provider name (github, antigravity, codex, claude)
...
export function parseQuotaData(provider: any, data: any) {
  ...
  try {
    switch (provider.toLowerCase()) {
      ...
      case "antigravity":
        if (data.quotas) {
          ...
          const geminiModels = entries.filter(
            ([k]: [string, any]) => k.startsWith("gemini-") && !k.includes("image"),
          );
          const claudeModels = entries.filter(([k]: [string, any]) => k.startsWith("claude-"));
          const imageModels = entries.filter(([k]: [string, any]) => k.includes("image"));
          const weeklyModels = entries.filter(([k]: [string, any]) => weeklyKeys.has(k));
          ...
          if (geminiModels.length > 0) {
            const rep = geminiModels.reduce((min: any, cur: any) =>
          ...
          if (claudeModels.length > 0) {
            const rep = claudeModels.reduce((min: any, cur: any) =>
              (cur[1].remainingPercentage ?? 100) < (min[1].remainingPercentage ?? 100) ? cur : min,
            )[1] as any;
            normalizedQuotas.push({
          ...
          weeklyModels.forEach(([modelKey, quota]: [string, any]) => {
            normalizedQuotas.push({
          ...
          imageModels.forEach(([modelKey, quota]: [string, any]) => {
            normalizedQuotas.push({
              name: quota.displayName || modelKey,
              modelKey,
              used: quota.used || 0,
          ...
          otherModels.forEach(([modelKey, quota]: [string, any]) => {
            normalizedQuotas.push({
              name: quota.displayName || modelKey,
              modelKey,
              used: quota.used || 0,
      ...
      case "groq":
        ...
        // used/total (calculatePercentage derives the bar), like Codex.
        if (data.quotas) {
          Object.entries(data.quotas).forEach(([name, quota]: [string, any]) => {
            normalizedQuotas.push({
              name,
  ...
  if (provider?.toLowerCase() === "claude") {
    ...
    normalizedQuotas.sort(
      (a: any, b: any) => (CLAUDE_QUOTA_ORDER[a.name] ?? 99) - (CLAUDE_QUOTA_ORDER[b.name] ?? 99),
    );
    return normalizedQuotas;
  }

  // Sort quotas according to PROVIDER_MODELS order
  const modelOrder: any = getModelsByProviderId(provider);
  if (modelOrder.length > 0) {
    const orderMap: Map<any, any> = new Map(modelOrder.map((m: any, i: any) => [m.id, i]));
