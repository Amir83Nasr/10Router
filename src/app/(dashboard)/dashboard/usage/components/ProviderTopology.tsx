  ...
  return (
    <div
      className={`relative z-[1] flex items-center justify-center px-5 py-3 rounded-xl border-2 min-w-[130px] ${
        powering
          ? "topo-router-core border-yellow-300 bg-gradient-to-br from-primary/30 via-yellow-400/20 to-cyan-400/25"
    ...
    >
      ...
      <img
        src="/favicon.svg"
        alt="10Router"
        className={`w-6 h-6 mr-2 ${powering ? "topo-router-icon" : ""}`}
        loading="lazy"
        decoding="async"
      />
      <span
        className={`text-sm font-bold ${powering ? "topo-router-label text-yellow-300" : "text-primary"}`}
      >
        10Router
      </span>
      {data.activeCount > 0 && (
        <span className="ml-2 px-1.5 py-0.5 rounded-full bg-yellow-400 text-black text-xs font-bold topo-router-badge">
          {data.activeCount}
        </span>
      )}
    </div>
...
function TopologyEdge({
  ...
  const stroke = style.stroke || "var(--border)";
  ...
  return (
    <g className="topo-edge-electric">
      <defs>
...
function buildLayout(providers, activeSet, lastSet, errorSet) {
  ...
  const edgeStyle = (active, last, error) => {
    ...
    return { stroke: "var(--border)", strokeWidth: 1, opacity: 0.3 };
  };
...
export default function ProviderTopology({
  ...
  return (
    <div
      ref={containerRef}
      className="flex h-[480px] w-full min-w-0 flex-col overflow-hidden rounded-lg border border-border bg-card"
    ...
    >
      ...
      <div className="min-h-0 flex-1">
        {providers.length === 0 ? (
          ...
          >
            <Controls showInteractive={false} className="topo-flow-controls" />
          </ReactFlow>
        )}
      </div>
    </div>
