...
const getClaudeSettingsPath = () => path.join(hostHome(), ".claude", "settings.json");

// Claude Code CLI reads mcpServers from ~/.claude.json (NOT settings.json).
const getClaudeJsonPath = () => path.join(hostHome(), ".claude.json");
...
export async function GET() {
  try {
    ...
    if (!isInstalled) {
      // Inside a container the server can't see host CLIs — the tool may be
      // installed on the host. Say "manual setup", not "not installed".
