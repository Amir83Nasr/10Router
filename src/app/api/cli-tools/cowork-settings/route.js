...
// Roots come from ALL platforms: the container reports linux, but the host
// behind HOST_HOME may be macOS/Windows. hostHome() points there when set.
...
const getAppInstallPaths = () => {
  // HOST_HOME mounts shadow the container root — skip global paths there.
  if (hasHostHome()) return [path.join(hostHome(), "Applications", "Claude.app")];
  if (os.platform() === "darwin") {
    return ["/Applications/Claude.app", path.join(hostHome(), "Applications", "Claude.app")];
  }
  if (os.platform() === "win32") {
    const localApp = process.env.LOCALAPPDATA || path.join(hostHome(), "AppData", "Local");
...
const get1pRoot = () => {
  if (hasHostHome()) return path.join(hostHome(), "Library", "Application Support", "Claude");
  if (os.platform() === "darwin")
    return path.join(hostHome(), "Library", "Application Support", "Claude");
  if (os.platform() === "win32") {
    const roaming = process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming");
    return path.join(roaming, "Claude");
  }
  return path.join(hostHome(), ".config", "Claude");
};
    ...
    if (!installed) {
      if (isContainer() && !hasHostHome()) {
        return NextResponse.json({
          ...
          container: true,
          config: null,
          message: "Running in a container — configure the app on the host manually",
        });
      }
      return NextResponse.json({
        installed: false,
