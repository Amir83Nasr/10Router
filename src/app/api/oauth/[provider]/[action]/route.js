  ...
  try {
    ...
    if (action === "authorize") {
      ...
      if (provider === "xiaomi-mimo") {
        const { generateKeyPair, buildAuthorizeUrl, getKeyName } =
          await import("@/lib/oauth/providers/xiaomi-mimo");
...
export async function POST(request, { params }) {
  try {
    ...
    if (action === "exchange") {
      ...
      // Kimchi returns a browser token.
    ...
    if (action === "poll") {
      ...
      const noPkceProviders = ["github", "kilocode", "codebuddy-cn", "codebuddy-intl"];
      let result;
