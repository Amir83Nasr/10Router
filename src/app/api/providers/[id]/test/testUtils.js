...
async function probeCloudCodeAssistAccess(connection, accessToken, effectiveProxy = null) {
  const userAgent = "google-api-nodejs-client/9.15.1 vscode-antigravity/1.107.0";
...
async function testOAuthConnection(connection, effectiveProxy = null) {
  // Zed RSA native-app auth has no OAUTH_TEST_CONFIG entry by design (no
  // client_id/secret, no refresh URL). Probe GET /client/users/me — the same
  // endpoint the usage card hits — with the connection's own proxy.
  if (connection.provider === "zed") {
    if (!connection.accessToken)
      return { valid: false, error: "No access token", refreshed: false };
    const proxyOptions = effectiveProxy
      ? {
    ...
    try {
      ...
      return { valid: false, error: err.message || "Zed test failed", refreshed: false };
    }
  }
