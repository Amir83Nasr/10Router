...
export async function PUT(request, { params }) {
  try {
    ...
    // Re-apply the global outbound proxy when the selected pool itself changes.
    try {
      const settings = await getSettings();
      if (settings?.outboundProxyPoolId === id) {
        await applyResolvedOutboundProxy(settings);
      }
    } catch (err) {
      console.warn("[ProxyPools] Failed to re-apply outbound proxy:", err);
    }
...
export async function DELETE(request, { params }) {
  try {
    ...
    // Re-apply the global outbound proxy when its pool is deleted
    // (falls back to the manual URL fields) and clear the stale reference.
    try {
      const settings = await getSettings();
      if (settings?.outboundProxyPoolId === id) {
        const next = await updateSettings({ outboundProxyPoolId: "" });
        await applyResolvedOutboundProxy(next);
      }
    } catch (err) {
      console.warn("[ProxyPools] Failed to re-apply outbound proxy:", err);
    }
