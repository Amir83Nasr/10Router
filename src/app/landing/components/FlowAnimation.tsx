...
export default function FlowAnimation() {
  ...
  return (
    <div className="mt-16 w-full max-w-4xl relative h-[360px] hidden md:flex items-center justify-center animate-[float_6s_ease-in-out_infinite]">
      {/* 10Router Hub - Center */}
      <div className="relative z-20 w-32 h-32 rounded-full bg-[#122019] border-2 border-[#32ec83] shadow-[0_0_40px_rgba(50,236,131,0.3)] flex flex-col items-center justify-center gap-1 group cursor-pointer hover:scale-105 transition-transform duration-500">
        <span className="material-symbols-outlined text-4xl text-[#32ec83]">hub</span>
        <span className="text-xs font-bold text-white tracking-widest uppercase">10Router</span>
        <div className="absolute inset-0 rounded-full border border-[#32ec83]/30 animate-ping opacity-20"></div>
      </div>
      ...
      <div className="absolute left-0 top-1/2 -translate-y-1/2 flex flex-col gap-7">
        {CLI_TOOLS.map((tool) => (
          ...
          >
            <div className="w-16 h-16 rounded-2xl bg-[#122019] border border-[#1f2f26] flex items-center justify-center overflow-hidden p-2 hover:border-[#32ec83]/50 transition-all hover:scale-105">
              <ProviderIcon
                src={tool.image}
                alt={tool.name}
                size={48}
      ...
      >
        <path
          ...
          fill="none"
          stroke={activeFlow === 0 ? "#32ec83" : "rgb(75, 85, 99)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 1 ? "#32ec83" : "rgb(75, 85, 99)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 2 ? "#32ec83" : "rgb(75, 85, 99)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 3 ? "#32ec83" : "rgb(75, 85, 99)"}
        ...
        {PROVIDERS.map((provider, idx) => (
          <div
            ...
            className={`px-4 py-2 rounded-lg ${provider.color} ${provider.textColor} flex items-center justify-center font-bold text-xs shadow-lg hover:scale-110 transition-all cursor-help min-w-[140px] ${
              activeFlow === idx ? "ring-4 ring-[#32ec83]/50 scale-110" : ""
            }`}
            title={provider.name}
          >
            {provider.name}
      ...
      {/* Mobile fallback */}
      <div className="md:hidden mt-8 w-full p-4 rounded-lg bg-[#122019] border border-[#1f2f26]">
