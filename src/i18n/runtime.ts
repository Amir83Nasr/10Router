...
function getLocaleFromCookie(): string {
  ...
  const value = cookie ? decodeURIComponent(cookie.split("=").slice(1).join("=")) : DEFAULT_LOCALE;
  return normalizeLocale(value);
}

function applyLocaleToDom(locale: string): void {
...
// Process text node
type TextWithOriginal = Text & { _originalText?: string };

function processTextNode(node: TextWithOriginal): void {
