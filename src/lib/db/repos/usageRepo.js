...
export async function saveRequestUsage(entry) {
  try {
    ...
    db.transaction(() => {
      // Dedupe only when caller supplies an explicit timestamp (retry/idempotency
      // key). Auto-stamped parallel bursts share the same ms and must all insert.
      // ponytail: add explicit idempotency-key column when retries need dedupe.
      let existing = null;
