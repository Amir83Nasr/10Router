...
export default function McpMarketplaceModal({
  ...
  const [expandedUrl, setExpandedUrl] = useState<any>(null);
  const [toolsCache, setToolsCache] = useState<any>({});
  const [toolsLoading, setToolsLoading] = useState<any>({});
  ...
  const fetchTools = async (server: any) => {
    ...
    try {
      ...
      const toolNames = tools.length > 0 ? tools.map((t: any) => t.name) : fallback;
      setToolsCache((p) => ({
        ...p,
      ...
      setToolSelection((p) => ({
        ...p,
        [server.url]: Object.fromEntries(toolNames.map((t: any) => [t, true])),
      }));
    } catch (e) {
  ...
  const toggleTool = (url: any, tool: any) => {
    setToolSelection((prev) => ({ ...prev, [url]: { ...prev[url], [tool]: !prev[url]?.[tool] } }));
  };

  const setAllTools = (url: any, value: any) => {
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 px-2 py-1.5 bg-card rounded text-xs border border-border focus:outline-none focus:ring-1 focus:ring-primary/50"
          />
          <select
            value={filter}
            onChange={(e) => setFilter(e.target.value)}
            className="px-2 py-1.5 bg-card rounded text-xs border border-border focus:outline-none focus:ring-1 focus:ring-primary/50"
          >
                ...
                <div key={s.url} className="rounded border border-transparent hover:border-border">
                  <div className="flex items-start gap-2 px-2 py-2 hover:bg-black/5 dark:hover:bg-white/5">
                    {s.iconUrl ? (
                      ...
                      <div className="size-7 rounded bg-card shrink-0" />
                    )}
                  ...
                  {expanded && (
                    <div className="px-3 py-2 bg-card/40 border-t border-border flex flex-col gap-2">
                      {isLoadingTools && (
                        <div className="flex items-center gap-2 text-muted-foreground text-[10px] py-1">
                      ...
                      <button
                        ...
                        className="self-end px-2 py-1 rounded text-[10px] font-medium bg-primary text-primary-foreground hover:bg-primary/90"
                      >
                        ✓ Confirm Add
                      </button>
                    </div>
