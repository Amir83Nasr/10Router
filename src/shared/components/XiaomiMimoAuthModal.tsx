...
export default function XiaomiMimoAuthModal({
  isOpen,
  onSuccess,
  onClose,
}: XiaomiMimoAuthModalProps) {
  const [phase, setPhase] = useState<any>("detecting"); // detecting | found | not-found | importing | error
  const [detectResult, setDetectResult] = useState<any>(null);
  const [error, setError] = useState<any>(null);
  const [oauthUrl, setOauthUrl] = useState<any>(null);
          ...
          <>
            ...
            {!oauthUrl ? (
              ...
              <div className="flex flex-col gap-2">
                ...
                <Button onClick={handlePollOAuth} className="w-full">
                  Check Again
                </Button>
              </div>
            )}
