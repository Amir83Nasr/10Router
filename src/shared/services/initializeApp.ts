...
const g: AppSingleton = ((globalThis as any).__appSingleton ??= {
...
async function runHeavyStartup(): Promise<void> {
  ...
  if (settings.tunnelEnabled && !g.tunnelAutoResumed) {
    ...
    safeRestartTunnel("startup").catch((e: any) =>
  ...
  if (settings.tailscaleEnabled && !g.tailscaleAutoResumed) {
    ...
    safeRestartTailscale("startup").catch((e: any) =>
...
function hasQuotaAutoPingEnabled(settings: SettingsLike): boolean {
  return [settings?.claudeAutoPing, settings?.codexAutoPing].some((config: any) =>
...
function startWatchdog(): void {
  ...
  if ((g.watchdogInterval as any).unref) (g.watchdogInterval as any).unref();
}
...
function startNetworkMonitor(): void {
  ...
  if ((g.networkMonitorInterval as any).unref) (g.networkMonitorInterval as any).unref();
}
...
export function configureTunnelMonitoring(settings: SettingsLike): void {
