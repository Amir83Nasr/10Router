...
type PingHandler = {
  getUsage: (accessToken: string, proxyOptions: any) => Promise<{ quotas?: Record<string, any> }>;
  sendPing: (
    connection: any,
    providerConfig: any,
    proxyOptions: any,
...
const providerHandlers: Record<ProviderName, PingHandler> = {
  claude: {
    getUsage: getClaudeUsage as PingHandler["getUsage"],
    sendPing: sendClaudePing,
  },
  codex: {
    getUsage: getCodexUsage as PingHandler["getUsage"],
...
} = ((globalThis as any).__quotaAutoPing ??= {
  interval: null,
  running: false,
  resetCache: {},
  failureCache: {},
...
function normalizeResetKey(resetAt: unknown): string {
  const ms = new Date(resetAt as string).getTime();
  if (!Number.isFinite(ms)) return resetAt as string;
  return new Date(Math.floor(ms / 60000) * 60000).toISOString();
}

function getResetDriftMs(previousResetAt: unknown, nextResetAt: unknown): number {
  const previousMs = new Date(previousResetAt as string).getTime();
  const nextMs = new Date(nextResetAt as string).getTime();
...
function hasExhaustedBlockingQuota(quotas: Record<string, any>, sessionKey: string): boolean {
...
function buildProxyOptions(cfg: any) {
  return {
...
function buildCodexPingInput(text: string) {
  return [
    {
      type: "message",
      role: "user",
...
async function drainResponseBody(response: any): Promise<void> {
...
function shouldSkipAfterFailure(state: typeof g, key: string, nowMs: number = Date.now()): boolean {
...
export async function runQuotaAutoPingTick(
  ...
  try {
    ...
    for (const [provider, providerConfig] of Object.entries(C.providers)) {
      ...
      const handler = providerHandlers[provider as ProviderName];
      if (!handler) continue;

      const enabledMap = settings?.[typedConfig.settingsKey]?.connections || {};
      ...
      const targets = conns.filter(
        (conn: any) => conn.authType === "oauth" && enabledMap[conn.id] === true,
      );
...
export function startQuotaAutoPing(): void {
  ...
  if ((g.interval as any).unref) (g.interval as any).unref();
}
...
export function configureQuotaAutoPing(settings: any): void {
  const enabled = Object.values(C.providers).some((providerConfig) =>
    Object.values(
      (providerConfig as Record<string, any>).settingsKey
        ? settings?.[(providerConfig as Record<string, any>).settingsKey]?.connections || {}
        : {},
    ).some(Boolean),
  );
