...
import { handleAntigravityQuotaError, clearAntigravityStrikes } from "../services/antigravityQuota";
import { handleZedQuotaError, clearZedQuotaBlock } from "../services/zedQuota";
...
export async function handleChat(request, clientRawRequest = null) {
  ...
  // Request summary is emitted as the unified "REQ" line in chatCore (has fmt/thinking/account)
...
async function handleSingleModelChat(
  ...
  // Routing shown in the unified "REQ" line (client model → provider/model)
  ...
  while (true) {
    ...
    const result = await handleChatCore({
      ...
      onRequestSuccess: async () => {
        ...
        // A success proves the Zed account serves traffic — drop any stale quota block.
        clearZedQuotaBlock(credentials.connectionId);
      },
    });
    ...
    // Zed 402/403/429: refresh monthly quota (edit_predictions / model_requests)
    // to get the exact billing-cycle resetAt before locking
    ...
    if (
      provider === "zed" &&
      (result.status === 402 || result.status === 403 || result.status === 429)
    ) {
      quotaResetMs = await handleZedQuotaError(
        credentials.connectionId,
        result.status,
        model,
        refreshedCredentials.accessToken,
    ...
    // Same for exhausted Zed monthly quota (billing-cycle reset can be weeks out —
    // far beyond the 30m DB-lock cap). Do not persist a modelLock_* for this path.
    const shouldFallback =
      (provider === "antigravity" || provider === "zed") && quotaResetMs
        ? true
        : (
