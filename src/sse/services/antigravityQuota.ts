...
const quotaCache = new Map<
  string,
  Record<string, { remainingPercentage: number; resetAt?: string }>
>();
...
const strikeCounts = new Map<string, { count: number; windowStart: number }>(); // "connectionId|model" → { count, windowStart (anchored at first strike) }
 ...
 */
function applyActiveStrikeBlocks(
  connectionId: string,
  quotas: Record<string, { remainingPercentage: number; resetAt?: string }>,
) {
  const now = Date.now();
 ...
 */
export async function refreshAntigravityQuota(
  connectionId: string,
  accessToken: string,
  providerSpecificData: any,
): Promise<Record<string, { remainingPercentage: number; resetAt?: string }> | null> {
  const now = Date.now();
