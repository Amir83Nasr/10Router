...
import { getAntigravityQuotaCache } from "./antigravityQuota";
import { getZedQuotaCache } from "./zedQuota";
...
function githubMonthlyResetMs(
  status: string | number,
  errorText: string | null,
  provider: string | null,
): number | null {
  if (resolveProviderId(provider as string) !== "github" || Number(status) !== 402) return null;
  if (
    !String(errorText || "")
      .toLowerCase()
...
export async function getProviderCredentials(
  ...
  options: { preferredConnectionId?: string } = {},
): Promise<any> {
  ...
  try {
    ...
    if (FREE_PROVIDERS[providerId]?.noAuth) {
      ...
      if (strategy !== "none") {
        ...
        const poolIds = allPools.filter((p) => p.proxyUrl).map((p: any) => p.id);
    ...
    // Zed monthly-quota cache is lazy: only populated after that account returns 402/403/429.
    const isZed = providerId === "zed";
    const zedQuotaCache = isZed ? getZedQuotaCache() : null;
    ...
    const availableConnections = connections.filter((c: any) => {
      ...
      // Zed: skip if monthly quota exhausted for this account (account-level,
      // not per-model — the quota rows are edit_predictions / model_requests).
      if (isZed && zedQuotaCache) {
        const blocked = zedQuotaCache.get(c.id);
        if (
          blocked &&
          (blocked.remainingPercentage ?? 0) <= 0 &&
          blocked.resetAt &&
          new Date(blocked.resetAt).getTime() > Date.now()
        ) {
          const account = c.id?.slice(0, 8) || "unknown";
          log.info(
            "ZED_QUOTA",
            `${account} | CACHE_BLOCK — monthly quota exhausted until ${blocked.resetAt}`,
          );
          return false;
        }
      }
    ...
    if (availableConnections.length === 0) {
      ...
      const lockedConns = connections.filter((c: any) => isModelLockActive(c, model));
      const expiries: any[] = lockedConns
        .map((c: any) => getEarliestModelLockUntil(c))
        .filter(Boolean);
      if (isAntigravity && model && antigravityQuotaCache) {
        connections.forEach((c: any) => {
      ...
      if (isZed && zedQuotaCache) {
        connections.forEach((c: any) => {
          const resetAt = zedQuotaCache.get(c.id)?.resetAt;
          if (resetAt && new Date(resetAt).getTime() > Date.now()) expiries.push(resetAt);
        });
      }
    ...
    if (connection) {
      ...
      // Bound lastUsed overlay scan to a 2-day window — stale timestamps
      // outside it are treated as never-used so old rows can't pin ordering.
      const twoDaysAgo = Date.now() - 2 * 24 * 60 * 60 * 1000;
      const lastUsedMs = (c: any) => {
        const t = c?.lastUsedAt ? new Date(c.lastUsedAt).getTime() : NaN;
        return Number.isFinite(t) && t >= twoDaysAgo ? t : NaN;
      };

      // Sort by lastUsed (most recent first) to find current candidate
      const byRecency = [...availableConnections].sort((a: any, b: any) => {
        const at = lastUsedMs(a);
        const bt = lastUsedMs(b);
        if (Number.isNaN(at) && Number.isNaN(bt)) return (a.priority || 999) - (b.priority || 999);
...
export async function markAccountUnavailable(
  ...
  if (githubResetAtMs) {
    ...
    cooldownMs =
      resolveProviderId(provider as string) === "antigravity"
        ? resetsAtMs - Date.now()
  ...
  if (provider && status && reason) {
    log.error(provider, `[${status}]: ${reason}`);
  }
