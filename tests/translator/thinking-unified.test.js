...
describe("applyThinking per provider format", () => {
  ...
  it("Kimi on → reasoning_effort", () => {
    const out = apply("openai", "kimi-k2.6", { reasoning_effort: "high" }, "openai");
    expect(out.reasoning_effort).toBe("high");
  });
  it("Kimi auto → supported reasoning_effort", () => {
    const out = apply("openai", "kimi-k2.7", { reasoning_effort: "auto" }, "openai");
    expect(out.reasoning_effort).toBe("high");
  });
  it("Kimi unsupported OpenAI levels → supported reasoning_effort", () => {
    const minimal = apply("openai", "kimi-k2.7", { reasoning_effort: "minimal" }, "openai");
    const xhigh = apply("openai", "kimi-k2.7", { reasoning_effort: "xhigh" }, "openai");
  ...
  it("keeps Codex-only GPT-5.6 levels out of generic OpenAI translation", () => {
    const out = apply("openai", "gpt-5.6-sol", { reasoning_effort: "max" }, "openai");
    expect(out.reasoning_effort).toBe("xhigh");
  });
  ...
  it("Gemini model over its native format (antigravity) still gets generationConfig", () => {
    const out = apply(
      "antigravity",
