...
describe("checkFallbackError — request-scoped vs account-scoped failures", () => {
  ...
  it("still falls back for account-scoped statuses", () => {
    for (const status of [401, 402, 403, 404, 429, 451]) {
      expect(checkFallbackError(status, "nope").shouldFallback).toBe(true);
    }
  });

  it("cools the account down on a geo-block so the next account gets a chance", () => {
    const result = checkFallbackError(
      451,
      "[451]: Access to OpenAi models is not available in your region (IR)",
    );
