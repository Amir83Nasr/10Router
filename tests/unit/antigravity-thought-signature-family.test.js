...
const { geminiToOpenAIResponse } =
  await import("../../open-sse/translator/response/gemini-to-openai.js");
const { DEFAULT_THINKING_CLOUD_CODE_SIGNATURE } =
...
describe("antigravity thought signatures are scoped to the model family", () => {
  ...
  it("switching Claude -> Gemini mid-conversation sends the default signature, not Claude's", () => {
    ...
    expect(functionCallSignatures(req)).toEqual([DEFAULT_THINKING_CLOUD_CODE_SIGNATURE]);
  });
