...
describe("BaseExecutor.execute — retry by status (config-driven)", () => {
  ...
  it("retries a plain 500 then succeeds", async () => {
    const ex = makeExec({ baseUrl: "https://x/api", retry: { 500: { attempts: 2, delayMs: 0 } } });
...
describe("BaseExecutor.execute — computeRetryDelay hook veto", () => {
  it("only invokes computeRetryDelay when status has retry config", async () => {
    const ex = makeExec({
      baseUrl: "https://x/api",
      retry: { 500: { attempts: 0 }, 503: { attempts: 1, delayMs: 0 } },
    });
