...
describe("OpenAI usage pass-through", () => {
  it("passes through plain input/output when no cache fields are present", () => {
    const out = toOpenAIUsage({ inputTokens: 100, outputTokens: 50 }, "openai");
    expect(out.prompt_tokens).toBe(100);
    expect(out.completion_tokens).toBe(50);
  ...
  it("forward-compat: surfaces cache fields if the event shape grows them", () => {
    // ponytail: generic upstream doesn't expose cache today, but if it starts
