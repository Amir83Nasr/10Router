...
describe("Codex Refresh Token", () => {
  ...
  describe("getRefreshLeadMs (early refresh config)", () => {
    it("should return provider-specific lead time for OAuth providers", async () => {
      ...
      // removed providers fall back to the default buffer
      expect(getRefreshLeadMs("kimi")).toBe(TOKEN_EXPIRY_BUFFER_MS);
      expect(getRefreshLeadMs("kimi-coding")).toBe(TOKEN_EXPIRY_BUFFER_MS);
    });
