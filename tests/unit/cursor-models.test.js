...
describe("Cursor live model catalog", () => {
  ...
  it("fetches the account-specific catalog and caches it", async () => {
    ...
    const postProto = vi.fn().mockResolvedValue({ status: 200, body: Buffer.from(payload) });
    const credentials = {
      accessToken: "cursor-token",
      providerSpecificData: { machineId: "machine-id" },
    };

    await expect(resolveCursorModels(credentials, {}, { postProto })).resolves.toEqual({
    ...
    expect(postProto).toHaveBeenCalledTimes(1);
    expect(postProto).toHaveBeenCalledWith(
  ...
  it("fails open when the Cursor catalog request fails", async () => {
    const postProto = vi.fn().mockResolvedValue({ status: 403, body: Buffer.alloc(0) });
