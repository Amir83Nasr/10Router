...
// ─── Mock the executors/index.js to avoid transitive heavy deps ─────────────
// We mock the whole executor layer.
...
describe("buildEmbeddingsUrl", () => {
  ...
  it("unsupported provider (e.g. github-copilot) → 400 error, no fetch called", async () => {
    const result = await handleEmbeddingsCore(
      makeOptions({
        modelInfo: { provider: "github-copilot", model: "some-embedding" },
