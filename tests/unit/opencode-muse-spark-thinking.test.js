...
describe("OpenCode Free Muse Spark thinking", () => {
  ...
  it("routes Union Alpha through Anthropic Messages", () => {
    ...
    // Registry declares union-alpha with targetFormat "claude"; URL routing
    // still lives in the executor's MESSAGES_MODELS set.
  ...
  it("strips prior-turn reasoning items carrying encrypted_content from input", () => {
    ...
    // Tools flattened and empty properties added; decoy tools cloaked on
    // every Responses request (d6f32f85) to stop 403 FreeTierError.
    ...
    expect(out.tools.map((t) => t.name)).toEqual(expect.arrayContaining(["shell", "bash", "read"]));
  });
});
