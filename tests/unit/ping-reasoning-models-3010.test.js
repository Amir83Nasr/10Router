...
vi.mock("@/shared/constants/config", () => ({ APP_PORT: 20127 }));
...
describe("pingModelByKind reasoning models (#3010)", () => {
  ...
  it("uses a 1024-token budget for the chat completions probe", async () => {
    ...
    await pingModelByKind("openai/o3-mini", "llm", "http://127.0.0.1:20127");
  ...
  it("treats a reasoning-only (length-limited) response as ok:true", async () => {
    ...
    const result = await pingModelByKind("openai/o3-mini", "llm", "http://127.0.0.1:20127");
