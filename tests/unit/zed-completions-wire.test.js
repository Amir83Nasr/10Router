...
describe("completion payload shaping", () => {
  ...
  it("maps HTTP 451 geo-block to a 451 with proxy guidance", async () => {
    const executor = makeExecutor();
    ...
    const parsed = executor.parseError(
      response,
      JSON.stringify({ message: "Access to OpenAi models is not available in your region (IR)" }),
    );
    expect(parsed.status).toBe(451);
    expect(parsed.message).toMatch(/geo-blocked/i);
    expect(parsed.message).toMatch(/egress proxy/i);
  });
});
